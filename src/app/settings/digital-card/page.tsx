...
export default function DigitalCardSettings() {
      ...
      <AppLayout title={t('digitalCard.title')} showBack={true} showTabs={true}>
        <div className="flex items-center justify-center h-full">
          <div className="text-center py-12">
            ...
            <div className="text-lg" style={{ color: '#0F172A' }}>{t('digitalCard.loading')}</div>
          </div>
        </div>
      </AppLayout>
    );
  ...
  // Componente preview de la tarjeta — reutilizado en mobile (arriba del form) y desktop (columna derecha)
  const CardPreview = () => (
    <div className="rounded-2xl overflow-hidden shadow-lg" style={{ backgroundColor: '#FFFFFF' }}>
      {/* Cover Photo */}
      <div className="relative h-32 bg-gradient-to-br from-blue-500 to-purple-600">
        {formData.cover_photo && (
          <Image src={formData.cover_photo} alt="Cover" fill className="object-cover" style={{ zIndex: 0 }} />
        )}
        ...
        <button
          ...
          className="absolute bottom-2 left-2 px-3 py-1 rounded-lg text-sm font-bold text-white cursor-pointer shadow-lg"
        ...
        <div className="absolute top-2 right-2 px-2 py-1 rounded text-xs font-semibold" style={{ backgroundColor: 'rgba(255,255,255,0.95)', color: '#2563EB', zIndex: 10 }}>
          📐 1200×400px
        </div>
      </div>
      ...
      <div className="relative px-4 pb-4">
        <div className="flex items-end gap-4 -mt-12">
          <div className="relative">
            <div className="w-24 h-24 rounded-full border-4 border-white bg-gray-200 overflow-hidden shadow-lg">
              {formData.profile_photo ? (
                <Image src={formData.profile_photo} alt="Profile" width={96} height={96} className="object-cover w-full h-full" />
              ) : (
            ...
            <div className="absolute -top-1 -left-1 px-1.5 py-0.5 rounded shadow-md" style={{ backgroundColor: '#FFFFFF', color: '#2563EB', fontSize: '9px', fontWeight: 700 }}>
              400×400px
            </div>
          </div>
          <div className="flex-1 mt-2">
            ...
            {formData.brokerage && (
              <p className="text-sm opacity-70" style={{ color: '#0F172A' }}>{formData.brokerage}</p>
            )}
          </div>
        </div>
          ...
          <div className="mt-4 pt-4 border-t" style={{ borderColor: '#E5E7EB' }}>
            <div className="flex justify-center gap-3">
              {formData.facebook_url && (
                <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-md" style={{ backgroundColor: '#1877F2' }}>
                  <span className="text-white font-bold">f</span>
                </div>
              )}
              {formData.instagram_url && (
                <div className="w-10 h-10 rounded-full flex items-center justify-center shadow-md" style={{ background: 'linear-gradient(45deg, #f09433 0%,#e6683c 25%,#dc2743 50%,#cc2366 75%,#bc1888 100%)' }}>
  ...
  return (
    <AppLayout title={t('digitalCard.title')} showBack={true} showTabs={true}>
      <form onSubmit={handleSubmit}>
        ...
        */}
        <div className="p-4 md:p-6 md:grid md:grid-cols-2 md:gap-6 md:items-start lg:grid-cols-[1fr_400px]">

          {/* ── COLUMNA IZQUIERDA — formulario ── */}
          <div className="space-y-6">

            {/* Preview — solo visible en mobile */}
            ...
            <div className="md:hidden space-y-2 pb-6">
              <button
                ...
                disabled={!username}
                className="w-full py-4 rounded-xl font-bold border-2 active:scale-95 transition-transform"
