...
export async function exportPropertyToPDF(property: any, agentParam?: AgentInfo, customFieldsDefinitions?: any[], propertyLanguage?: 'es' | 'en', currency?: { symbol: string; code: string }) {
  ...
  pdf.addPage();
  await createCompactDetailsPage(pdf, property, agent, pageWidth, pageHeight, margin, customFieldsDefinitions, t, lang);
...
async function createCompactCoverPage(
  ...
  // Usar símbolo de centavos ¢ para colones (compatible con jsPDF)
  let currencySymbol = currency?.symbol || '$';
  if (currencySymbol === '₡' || currency?.code === 'CRC') {
    currencySymbol = '¢';
  }
