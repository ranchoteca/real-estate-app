...
// NOTE: description is intentionally excluded — it is never shown in the summary
// and is always generated by AI in handleConfirmacion() before property creation.
function parseSummaryText(summaryText: string, draftPhotos: string[], draftCustomFields: Record<string, string | number>): Partial<PropertyDraft> {
  const getField = (emoji: string): string | null => {
  ...
  const typeLabel = getField('🏷️');
  const listingLabel = getField('📋');
  const priceRaw = getField('💰');
  const province = getField('📍\\s*\\*?Provincia');
  const city = getField('🌆');
  const address = getField('🏠\\s*\\*?Dirección');
  const mapsUrl = getField('📍\\s*\\*?Google Maps');
  const languageLabel = getField('🌐');

  // Reverse map display label → property_type key
  ...
  // Price: strip dots/spaces used as thousand separators, then extract the number
  ...
  if (priceRaw) {
    ...
    if (/USD/i.test(priceRaw)) {
      currency_id = '839f44d5-bee2-4bc1-b5da-50364f14c681';
    } else if (/CRC|₡|colones/i.test(priceRaw)) {
  ...
  return {
    ...
    property_type: typeLabel ? (typeReverse[typeLabel] || 'other') : undefined,
    listing_type: listingLabel ? (listingReverse[listingLabel] || 'sale') : undefined,
    price: price || undefined,
    currency_id: currency_id || undefined,
    state_province: province || undefined,
    city: city || undefined,
    address: (address && address !== 'No indicada') ? address : undefined,
    maps_url: (mapsUrl && mapsUrl !== 'No indicado') ? mapsUrl : undefined,
    language: languageLabel === 'Inglés' ? 'en' : 'es',
    photos: draftPhotos,
    pending_photos: draftPhotos.length,
    custom_fields_data: draftCustomFields,
  };
}

// Generates a natural, non-generic property description using GPT based on all
// available property data. Called when description is missing from the draft
// (either because the agent never provided one, or due to the race condition).
async function generateDescription(draft: PropertyDraft): Promise<string> {
  ...
  const currencyLabel = draft.currency_id === '839f44d5-bee2-4bc1-b5da-50364f14c681' ? 'USD' : '₡';
  const priceFormatted = draft.price ? draft.price.toLocaleString('es-CR') + ' ' + currencyLabel : null;
  const typeLabel = typeMap[draft.property_type || ''] || 'Propiedad';
  const listingLabel = listingMap[draft.listing_type || ''] || '';

  // Build context block from all available fields including custom fields
  const contextLines: string[] = [
    'Tipo: ' + typeLabel + ' ' + listingLabel,
    draft.city ? 'Ubicación: ' + draft.city + (draft.state_province ? ', ' + draft.state_province : '') : null,
    draft.address ? 'Dirección: ' + draft.address : null,
    priceFormatted ? 'Precio: ' + priceFormatted : null,
  ].filter(Boolean) as string[];

  // Append custom fields as additional context
  if (draft.custom_fields_data && Object.keys(draft.custom_fields_data).length > 0) {
    Object.entries(draft.custom_fields_data).forEach(([key, val]) => {
  ...
  try {
    ...
    return completion.choices[0].message.content?.trim() || typeLabel + ' ' + listingLabel + ' en ' + (draft.city || 'Costa Rica') + '.';
  } catch (err) {
    console.error('[generateDescription] OpenAI error:', err);
    // Fallback: minimal description so the not-null constraint is always satisfied
    return typeLabel + ' ' + listingLabel + ' en ' + (draft.city || 'Costa Rica') + '.';
  }
}
...
export async function handleConfirmacion(
  ...
  let draftToCreate: PropertyDraft = draft;
  ...
  if (!draft.title || !draft.property_type || !draft.listing_type || !draft.price || !draft.city) {
    ...
    const fromSummary = parseSummaryText(lastBotContent, draft.photos || [], draft.custom_fields_data || {});
    draftToCreate = { ...draft, ...fromSummary };

    // Persist the recovered fields so the draft is consistent going forward
    await upsertDraft(agentId, fromSummary);
  }

  // Final check: if we still can't get the critical fields, abort gracefully
  if (!draftToCreate.title || !draftToCreate.property_type || !draftToCreate.listing_type || !draftToCreate.price || !draftToCreate.city) {
    console.error('[handleConfirmacion] Could not recover critical fields from summary:', {
      title: draftToCreate.title,
      property_type: draftToCreate.property_type,
      listing_type: draftToCreate.listing_type,
      price: draftToCreate.price,
      city: draftToCreate.city,
    });
    await sendQueued(agentId, cleanNumber,
  ...
  // ── Generate description with AI if missing ───────────────────────────────
  // Description is not shown in the summary so it may be null from the race
  // condition or simply never provided by the agent. Always generate it here
  // so the not-null constraint in properties is always satisfied.
  if (!draftToCreate.description) {
    draftToCreate.description = await generateDescription(draftToCreate);
  }

  await sendQueued(agentId, cleanNumber, '⏳ Perfecto ' + primerNombre + ', creando tu propiedad... Dame un momento.');
  await clearDraft(agentId);
  await crearPropiedad(agentId, cleanNumber, draftToCreate);
}
